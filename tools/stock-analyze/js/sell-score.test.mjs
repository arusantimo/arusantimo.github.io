...
test('수급 매집형 시가 미회복은 시간 기반 강등 없이 일반 점수로 계산된다', () => {
  const { buildSellExecutionContext } = loadSellScoreContext();
  const result = buildSellExecutionContext({
    stock: { type: 'momentum' },
    payload: {
  ...
  assert.ok(result.sellScore < 70);
  assert.equal(result.actionPlan.bucket, 'hold');
  assert.equal(result.actionPlan.sellNowPct, 0);
  assert.equal(result.actionPlan.holdPct, 100);
  assert.equal(result.actionPlan.recheckLabel, '다음 분석 시점 재평가');
});
