...
echo "  ============================================================"
echo ""
echo "  [SH] replay-only 검증 + 화면 브리지 갱신"
echo ""

if [[ ${#ARGS[@]} -gt 0 ]]; then
  "$PYTHON" "$SCRIPT_DIR/run_jongga_pipeline.py" --replay-only "${ARGS[@]}"
else
  "$PYTHON" "$SCRIPT_DIR/run_jongga_pipeline.py" --replay-only
fi
EXITCODE=$?
